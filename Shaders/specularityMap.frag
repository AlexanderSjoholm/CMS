...
void main(void)
{
	...
	vec4 texel0 = texture(Tex0, texFix);
	vec4 texel1 = texture(Tex1, texFix);
	vec4 texel2 = texture(Tex2, texFix);
	vec4 texel3 = texture(Tex3, texFix);
	
	
	vec3 totalLight = (ambientCoeff + (diffuseCoeff*diffuseComponent + texel2[0]*specularCoeff*specularComponent) )*lightColor;
	
	
	//vec3 color = vec3(1, 1, 1);
	
	//out_Color = vec4(totalLight, 1)*( shadow*texel0 + (1 - shadow)*texel1 );
	out_Color = ( shadow*texel0 + (1 - shadow)*texel1 );
