...
static struct Mesh * LoadOBJ(const char *filename)
{
  ...
  std::cout << "LoadOBJ 2" << std::endl;

  fopen_s(&fp, filename, "r");
  //fp = fopen(filename, "rw");

  std::cout << "LoadOBJ 3" << std::endl;
  ...
  std::cout << "LoadOBJ 5.1" << std::endl;

  if (fp == NULL) 
	  return NULL;
  else
	  std::cout << "Good Stream" << std::endl;
  std::cout << "LoadOBJ 5.2" << std::endl;
  ParseOBJ(theMesh);
  std::cout << "LoadOBJ 5.3" << std::endl;
  //fclose(fp);
  std::cout << "LoadOBJ 5.4" << std::endl;
...
void DecomposeToTriangles(struct Mesh *theMesh)
{
  ...
  std::cout << "Decomp 3.1" << std::endl;
  //free(theMesh->coordIndex);
  std::cout << "Decomp 3.2" << std::endl;
  theMesh->coordIndex = newCoords;
  std::cout << "Decomp 3.3" << std::endl;
  theMesh->coordCount = triangleCount * 3;
  std::cout << "Decomp 3.4" << std::endl;
  if (theMesh->normalsIndex != NULL)
    {
		std::cout << "Decomp 3.4.1" << std::endl;
      //free(theMesh->normalsIndex);
	  std::cout << "Decomp 3.4.2" << std::endl;
      theMesh->normalsIndex = newNormalsIndex;
	  std::cout << "Decomp 3.4.3" << std::endl;

    }
  if (theMesh->textureIndex != NULL)
    {
		std::cout << "Decomp 3.4.4" << std::endl;
      //free(theMesh->textureIndex);
	  std::cout << "Decomp 3.4.5" << std::endl;
      theMesh->textureIndex = newTextureIndex;
	  std::cout << "Decomp 3.4.6" << std::endl;

    }
  std::cout << "Decomp 4" << std::endl;
  ...
  if (mesh->vertices && !mesh->vertexNormals)
    {
	  ...
	  mesh->vertexNormals = new GLfloat;
...
static Model* generateModel(Mesh* mesh)
{
  ...
  // and where new vertices have been created whenever necessary
	...
	std::cout << "Generate 1.1" << std::endl;
  typedef struct
  {
    int positionIndex;
    int normalIndex;
  ...
  std::cout << "Generate 1.2" << std::endl;

  IndexTriplet* indexHashMap = new IndexTriplet;
  //IndexTriplet* indexHashMap = malloc(sizeof(IndexTriplet) * indexHashMapSize);
  ...
  std::cout << "Generate 4" << std::endl;
  free(indexHashMap);

  std::cout << "Generate 5" << std::endl;
  return model;
}
...
{
  Model* model = 0;
  std::cout << "LoadModel 1" << std::endl;
  Mesh* mesh = LoadOBJ(name);

  std::cout << "LoadModel 2" << std::endl;
  ...
  std::cout << "LoadModel 3" << std::endl;

  generateNormals(mesh);

  std::cout << "LoadModel 4" << std::endl;

  model = generateModel(mesh);

  std::cout << "LoadModel 5" << std::endl;
			...
			char* texCoordVariableName)
{
	...
	std::cout << "LoadModelPlus 1" << std::endl;
	...
	std::cout << "LoadModelPlus 2" << std::endl;

	glBindVertexArray(m->vao);
    printError("TexError1");
	// VBO for vertex data
    ...
    printError("TexError2");
	std::cout << "LoadModelPlus 3" << std::endl;
	// VBO for normal data
    ...
    printError("TexError3");
	std::cout << "LoadModelPlus 4" << std::endl;
	...
	if (m->texCoordArray != NULL)
	{
		...
		printError("TexError4");
		std::cout << "LoadModelPlus 5" << std::endl;
	}
